<template>
  <div class="managemusic">
    ...
    <Row style="padding:20px 60px 20px 40px;">
      ...
      <Col :span="3">
        <el-button type="primary" style="margin-left:5px;" @click="getAllSong()">所有音乐</el-button>
      </Col>
      <Col :span="4">
        <el-button type="success" style="margin-left:5px;" @click="toListen = true">播放音乐</el-button>
      </Col>
      <Col :span="12" style="text-align:right;">
        ...
        />
        <el-button type="primary" @click="showAddSongBtn()">添加</el-button>
      </Col>
    </Row>
    <!-- 歌曲数据表 -->
    <div style="width:100%;background-color:#f40;">
      <el-table :data="allSongs" class="song-table" style="width: 100%" border>
        ...
        <el-table-column label="音乐名" prop="songName" align="center" width="200"></el-table-column>
        ...
        <el-table-column align="center" prop="language" width="120" label="音乐语种"></el-table-column>
        <el-table-column align="center" prop="style" width="120" label="音乐风格"></el-table-column>
        <el-table-column align="center" prop="playcount" width="80" label="音乐播放次数"></el-table-column>
        <el-table-column align="center" prop="date" width="200" label="音乐最近更新时间"></el-table-column>
        <el-table-column prop="operation" align="center" label="音乐管理" fixed="right" min-width="150">
          <template slot-scope="scope">
            <el-button
              type="success"
              size="small"
    ...
    >
      <div class="form">
        ...
        >
          ...
          <el-form-item label="音乐封面:" prop="poster">
            ...
            >
              <span style="float:left;">
                <Avatar
                  :src="'http://localhost:8633/api/music/poster?img=' + addForm.poster"
                  size="large"
                />
              </span>
            </Upload>
          </el-form-item>
          <el-form-item prop="src" label="音乐文件">
            <Upload
              action="http://localhost:8633/api/admin/upload/music"
              :on-success="handleAddSongSuccess"
              ref="uploadmusic"
              :format="['mp3']"
            >
              <Button icon="ios-cloud-upload-outline" @click.native="clearFiles">上传音乐</Button>
            </Upload>
          </el-form-item>
          <el-form-item prop="language" label="音乐语种:">
          ...
          <el-form-item prop="style" label="音乐风格:">
    ...
    >
      <div class="form">
        <el-form
          ...
          style="margin:10px;width:auto;"
        >
          <el-form-item prop="_id" label="音乐ID:">
            <el-input type="id" disabled v-model="editForm._id"></el-input>
          </el-form-item>
          <el-form-item prop="songName" label="音乐:">
          ...
          <el-form-item prop="src" label="音乐地址:">
            <Upload
              action="http://localhost:8633/api/admin/upload/music"
          ...
          <el-form-item label="音乐封面:" prop="pic">
            <Upload
              :show-upload-list="false"
              :format="['jpg','jpeg','png']"
              :max-size="50000"
            ...
            >
              <span style="float:left;">
                <Avatar
                  :src="'http://localhost:8633/api/music/poster?img='+ editForm.poster"
                  size="large"
                />
              </span>
            </Upload>
    ...
    <Modal draggable v-model="isCheckPassword" :mask-closable="false" width="360">
      <p slot="header" style="color:#C20C0C;">
        <Icon type="el-icon-loading"></Icon>
        <span>管理员身份验证</span>
      </p>
      <div style="text-align:left;font-size:14px;">
        <Row>
          ...
          <Col span="6">
            <Button type="danger" size="large" @click="checkPwd()">身份验证</Button>
          </Col>
        </Row>
      </div>
      <div slot="footer"></div>
      ...
      <div v-if="toListen">
        <div style="width:100%;">
          <el-table :data="allSongs" height="300" style="width: 100%">
            ...
            <el-table-column align="center" width="200" label="播放音乐">
              <template slot-scope="scope">
                <button class="toListen-link" @click="listenMusic(scope.row)">播放</button>
              </template>
            </el-table-column>
          </el-table>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      // 语种
      Languages: ["汉语", "韩语", "粤语", "日语", "英语"],
      SongStyle: ["传统", "儿歌", "民歌", "摇滚", "伴奏", "古风", "流行"], // 歌曲风格
    };
  },
  created() {
    this.getAllSong();
  ...
  methods: {
    ...
    checkPwd() {
      if (this.inputPassword.trim().length) {
        ...
        wsmLoading.start("正在验证身份,请稍候...");
        setTimeout(() => {
          this.$axios
            .post("http://localhost:8633/api/admin/islegal", {
            ...
            .then((res) => {
              ...
              if (res.data == true) {
                if (this.flag == 1) {
                  ...
                  this.$axios
                    .post(
                      "http://localhost:8633/api/admin/music/add",
                      this.addForm
                    )
                    .then((res) => {
                      this.getAllSong();
                  ...
                  this.$axios
                    .post(
                      "http://localhost:8633/api/admin/music/edit",
                      this.editForm
                    )
                    .then((res) => {
                  ...
                  this.$axios
                    .post(
                      "http://localhost:8633/api/admin/music/delete",
                      this.delRow
                    )
                    .then((res) => {
                      console.log(res.data);
    ...
    likeSongs(song) {
      console.log(song);
      this.$confirm(
        `确定添加 ${song.songName} (${song.artist}) 歌曲到推荐歌曲里吗? `,
        "操作提示",
        {
        ...
        .then(() => {
          this.$axios
            .post("http://localhost:8633/api/admin/adminlike/add", {
              s_id: song._id,
            })
            .then((res) => {
    ...
    onDeleteSong(song) {
      this.$confirm(
        `确定删除 ${song.songName} (${song.artist}) 音乐吗?不可撤销,是否继续? `,
        "风险提示",
        {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
...
.ivu-modal-content {
  background-color: #c4b1b1 !important;
}
