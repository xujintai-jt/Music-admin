  ...
  <div class="user_service">
    <el-form :model="ruleForm" status-icon label-width="100px" class="ruleform">
      <el-form-item label="时长:" required>
        <el-select v-model="timeLong" clearable placeholder="请选择时长">
          <el-option v-for="item in timeOptions" :key="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
    ...
    <Row class="order-current" v-if="card.flag">
      <Card style="width:350px">
        <p slot="title">
          <Icon type="ios-checkmark-circle" style="color:green;" />开户成功
        </p>
        ...
        <div style="padding:5px 0px;">
          <Icon type="ios-contact" />账号:
          <span style="padding: 0px 2px;color:#19be6b;font-weight:700;">{{card.account}}</span>
        </div>
        <div style="padding:5px 0px;">
          <Icon type="ios-key" />密码:
          <span style="padding: 0px 2px;color:#19be6b;font-weight:700;">{{card.password}}</span>
        </div>
        <div style="padding:5px 0px;">
          <Icon type="logo-bitcoin" />金额:
          <span style="padding: 0px 2px;color:#ed4014;font-weight:700;">{{card.money}}元</span>
        </div>
        <div style="padding:5px 0px;">
          <Icon type="ios-copy" />订单编号:
          <span style="padding: 0px 2px;color:#2b85e4;">{{card.order_id}}</span>
        </div>
      </Card>
    </Row>
          ...
          this.$axios
            ...
            .then((res) => {
              if (res.data == true) {
                ...
                this.ruleForm.startTime = new Date().format(
                  "yyyy/MM/dd HH:mm:ss"
                );
                this.ruleForm.endTime = new Date(
                  this.timeLong * 60 * 60 * 1000 +
                    new Date(this.ruleForm.startTime).getTime()
                ).format("yyyy/MM/dd HH:mm:ss");
                this.$axios
                  .post(
                    "http://localhost:8633/api/admin/account/new",
                    this.ruleForm
                  )
                  .then((res) => {
                    this.card = { ...res.data };
                    this.card.flag = true;
                    this.card.currentTime = new Date().format(
                      "yyyy/MM/dd HH:mm:ss"
                    );
                    this.$nextTick(() => {
                      this.inputPassword = "";
