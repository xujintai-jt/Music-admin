  ...
  <div id="user-info">
    ...
    <div style="width:100%;background-color:#f40;">
      <el-table :data="userInfos" class="song-table" border>
        ...
        <el-table-column label="管理员id" prop="_id" align="center"></el-table-column>
        ...
        <el-table-column label="编辑" align="center">
          <template slot-scope="scope">
            <el-button icon="el-icon-edit" @click="editAdminInfo(scope.row)"></el-button>
          </template>
        </el-table-column>
        <el-table-column label="删除" align="center">
          <template slot-scope="scope">
            <el-button icon="el-icon-delete" @click="delnAdminInfo(scope.row)"></el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  methods: {
    ...
    getAdminInfo() {
      this.$axios
        .get(`http://localhost:8633/api/admin/query/all`)
        .then((res) => {
          const { data } = res;
          // console.log(data);
          if (data.status !== 200) {
      ...
      this.$refs[formName].validate(async (valid) => {
        if (valid) {
          ...
          const res = await this.$axios.post(
            "http://localhost:8633/api/admin/edit",
            this.ruleForm
          );
    ...
    delnAdminInfo(row) {
      this.$confirm("您确定删除此用户吗？", "删除提示", {
        ...
        .then(() => {
          const { _id } = row;
          this.$axios
            .post("http://localhost:8633/api/admin/admin/del", {
              _id,
            })
            .then((res) => {
              //刷新管理员数据
